...
- **Et à 100 collectivités ?** → évolution **par paliers** : 10-30 → BDD séparée + réplicas + CDN + cache · 30-60 → **load balancer** + multi-instances · 60-100 → orchestration/autoscaling + **renforts humains** (QA, support N1, 2e DevOps). Le goulot à 100 est **humain, pas technique** → **onboarding self-service** dès ~30. **Multi-tenant dès le départ** (non rattrapable après). Et passage aux **SLA** + astreinte facturée.

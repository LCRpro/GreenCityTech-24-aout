...
- **Pourquoi React Native ?** → **équipe de 4 devs qui font déjà du React** : même langage, mêmes composants → ils codent le mobile sans repartir de zéro, **1 base de code pour iOS + Android**. Écartés : **natif** (Swift+Kotlin = 2 bases, 2 profils à recruter, coût ×2) · **Flutter** (excellent mais **Dart** = tout réapprendre, zéro mutualisation avec le web) · **web mobile** (accès natif limité + pas de stores). Limites assumées : perfs < natif sur animations lourdes — *« mais notre app c'est un formulaire, une photo, une carte »*.
- **Un ou plusieurs dépôts ? Pourquoi un monorepo ?** → **Monorepo** (1 dossier par app + dossier partagé). ① **changements transverses** cohérents (1 PR, pas 4 → sinon une partie part en prod sans l'autre) · ② **partage réel** des types TS/validation (sinon duplication + dérive) · ③ **outillage homogène** (1 config ESLint/TS/tests, sinon divergence) · ④ 1 clone, historique unique. Multi-repo se justifie si **plusieurs équipes autonomes** — pas notre cas. Inconvénients assumés : dépôt qui grossit, **filtres par chemin** obligatoires, discipline anti-couplage.
